...
static int
euler_step (double *y, const euler_state_t * state,
            const double h, const double t, const size_t dim,
            const gsl_odeiv_system * sys)
{
  /* Makes a symplectic Euler advance with step size h */
  ...
  /* Solve p_{n+1} = p_{n} + h * (-1) * dH/dx(p_{n+1},q_{n}) for
     p_{n+1} */

  const int maxiter = 1e3;
  int iter = 1;

  const double normtol = GSL_DBL_EPSILON;
  double norm;

  /* Implicit step: advance in p with step size h */

  do
    {
      /* Method is explicit when H(p,q) = T(p) + V(q) */
  ...
  /* Explicit step: advance in q with step size h */

  for (size_t i = n; i < dim; i++) /* q[0] = y[n] ... */
    {
      y[i] = y0[i] + h * f[i];
    }
...
static int
euler_apply (void *vstate, size_t dim, double t, double h,
             ...
             double dydt_out[], const gsl_odeiv_system * sys)
{
  ...
  {
    int s = euler_step (y_onestep, state, h, t, dim, sys);
  ...
  {
    int s = euler_step (y, state, h / 2.0, t, dim, sys);
...

  {
    ...
    s = euler_step (y, state, h / 2.0, t + h / 2.0, dim, sys);
...
static const gsl_odeiv_step_type euler_type = {
  "euler",      /* name */
  1,            /* can use dydt_in */
  1,            /* gives exact dydt_out */
  &euler_alloc, /* state allocation */
  &euler_apply, /* advance one step */
  &euler_reset, /* reset to well-known state */
  &euler_order, /* return the order of the stepping function */
  &euler_free   /* free state */
};
