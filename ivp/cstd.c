...
{
  std_control_state_t * s =  (std_control_state_t *) malloc (sizeof (std_control_state_t));
...
static int
std_control_init (void * vstate, double eps_abs, double eps_rel, double a_y, double a_dydt)
{
...
std_control_hadjust (void * vstate, size_t dim, unsigned int ord, const double y[], const double yerr[], const double yp[], double * h)
{
  ...
  for (i = 0; i < dim; i++)
    {
      const double D0 = eps_rel * (a_y * fabs (y[i]) + a_dydt * fabs (h_old * yp[i])) + eps_abs;
...
gsl_odeiv_control *
gsl_odeiv_control_standard_new (double eps_abs, double eps_rel, double a_y, double a_dydt)
{
  gsl_odeiv_control * c = gsl_odeiv_control_alloc (gsl_odeiv_control_standard);
