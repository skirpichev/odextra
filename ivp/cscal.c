...
{
  sc_control_state_t * s = (sc_control_state_t *) malloc (sizeof (sc_control_state_t));

  if (s == 0)
    {
      GSL_ERROR_NULL ("failed to allocate space for sc_control_state", GSL_ENOMEM);
    }
...
static int
sc_control_init (void * vstate, double eps_abs, double eps_rel, double a_y, double a_dydt)
{
...
sc_control_hadjust (void * vstate, size_t dim, unsigned int ord, const double y[], const double yerr[], const double yp[], double * h)
{
  ...
  for (i = 0; i < dim; i++)
    {
      const double D0 = eps_rel * (a_y * fabs (y[i]) + a_dydt * fabs (h_old * yp[i])) + eps_abs * scale_abs[i];
...
{
  gsl_odeiv_control * c = gsl_odeiv_control_alloc (gsl_odeiv_control_scaled);
